...
hooks:
  ...
  Stop:
    - hooks:
        - type: command
          command: "sh \"$(ls ~/.config/opencode/skills/planning-with-files/scripts/check-complete.sh 2>/dev/null | head -1)\" 2>/dev/null || true"
metadata:
  version: "2.34.1"
---
...
The PLUGIN_ROOT_DIRECTORY is `~/.config/opencode/skills/planning-with-files`.
...
```bash
# Linux/macOS/WSL
$(command -v python3 || command -v python) ${PLUGIN_ROOT_DIRECTORY}/scripts/session-catchup.py "$(pwd)"
```
...
- **Templates** are in `${PLUGIN_ROOT_DIRECTORY}/templates/`
- **Your planning files** go in **your project directory**

| Location | What Goes There |
|----------|-----------------|
| Skill directory (`${PLUGIN_ROOT_DIRECTORY}/`) | Templates, scripts, reference docs |
